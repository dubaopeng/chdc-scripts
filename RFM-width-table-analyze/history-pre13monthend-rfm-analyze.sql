SET mapred.job.name='monthend-rfm-width-table-analyze-历史月底RFM宽表数据分析';
...
insert overwrite table dw_rfm.`b_first_buy_history_base` partition(part)
...
from(
	...
	from(
		...
		where substr(created,1,10) <= ${hiveconf:thisMonthEnd}
	) t where t.rank <= 2
) r
...
insert overwrite table dw_rfm.`b_last_year_history_base` partition(part)
select
	tenant,
    plat_code,
	uni_shop_id,
...
where created is not NULL
    and (created >= date_add(add_months(${hiveconf:thisMonthEnd},-12),1) and created <= ${hiveconf:thisMonthEnd})
group by
	tenant,
    plat_code,
	uni_shop_id,
...
insert overwrite table dw_rfm.`b_last_tyear_history_base` partition(part)
select
	tenant,
    plat_code,
	uni_shop_id,
...
from dw_rfm.b_rfm_history_base_trade
where created is not NULL
	and (created >= date_add(add_months(${hiveconf:thisMonthEnd},-24),1) and created < date_add(add_months(${hiveconf:thisMonthEnd},-12),1))
group by
	tenant,
    plat_code,
	uni_shop_id,
...
insert overwrite table dw_rfm.`b_before_tyear_history_base` partition(part)
select
	tenant,
    plat_code,
	uni_shop_id,
	...
	${hiveconf:thisMonthEnd} as part
from dw_rfm.b_rfm_history_base_trade
where created is not NULL and created < date_add(add_months(${hiveconf:thisMonthEnd},-24),1)
group by
	tenant,
    plat_code,
	uni_shop_id,
...
(
	...
	from 
	 (select * from dw_rfm.`b_first_buy_history_base` where part=${hiveconf:thisMonthEnd}) a
	 left join
	 (select * from  dw_rfm.`b_last_year_history_base` where part=${hiveconf:thisMonthEnd}) b
	 on a.tenant=b.tenant and a.plat_code = b.plat_code and a.uni_shop_id = b.uni_shop_id and a.uni_id = b.uni_id
	 left join
	 (select * from dw_rfm.`b_last_tyear_history_base` where part=${hiveconf:thisMonthEnd}) c
	 on a.tenant=c.tenant and a.plat_code = c.plat_code and a.uni_shop_id = c.uni_shop_id and a.uni_id = c.uni_id
	 left join
	 (select * from dw_rfm.`b_before_tyear_history_base` where part=${hiveconf:thisMonthEnd}) d
...
-- 去重获取平台级的最早购买时间
-- 店铺级的数据表定义 
CREATE TABLE IF NOT EXISTS dw_rfm.`plat_first_buy_history_all_temp`(
	`tenant` string,
	`plat_code` string,
	`uni_id` string,
	`first_buy_time` string,
...
insert overwrite table dw_rfm.`plat_first_buy_history_all_temp` partition(part)
...
from(
	...
	from(
		...
		where part=${hiveconf:thisMonthEnd}
	) t where t.rank <= 2
) r
...
(select tenant,plat_code,uni_id,first_buy_time,first_payment,second_buy_time 
	from dw_rfm.plat_first_buy_history_all_temp where part=${hiveconf:thisMonthEnd} 
) t
on r.tenant = t.tenant and r.plat_code=t.plat_code and r.uni_id=t.uni_id;

-- 去重排序，获取租户级的首次购买时间
CREATE TABLE IF NOT EXISTS dw_rfm.`tenant_first_buy_history_all_temp`(
	`tenant` string,
	`uni_id` string,
	`first_buy_time` string,
	`first_payment` double,
...
insert overwrite table dw_rfm.`tenant_first_buy_history_all_temp` partition(part)
...
(select tenant,uni_id,first_buy_time,first_payment,second_buy_time 
	from dw_rfm.tenant_first_buy_history_all_temp where part=${hiveconf:thisMonthEnd} 
) t 
