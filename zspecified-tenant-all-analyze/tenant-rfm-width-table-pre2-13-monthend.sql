...
from(
	...
	from(
		...
		where part='${tenant}'
		and substr(created,1,10) <= ${hiveconf:thisMonthEnd}
	) t where t.rank <= 2
) r
...
where part='${tenant}'
	and created is not NULL 
	and created < date_add(add_months(${hiveconf:thisMonthEnd},-24),1)
group by
	tenant,
    plat_code,
	uni_shop_id,
...
(
	...
	from 
	 (select * from dw_rfm.`b_first_buy_history_tenants` where part='${tenant}' and stat_date=${hiveconf:thisMonthEnd}) a
	 left join
	 (select * from  dw_rfm.`b_last_year_history_tenants` where part='${tenant}' and stat_date=${hiveconf:thisMonthEnd}) b
	 on a.tenant=b.tenant and a.plat_code = b.plat_code and a.uni_shop_id = b.uni_shop_id and a.uni_id = b.uni_id
	 left join
	 (select * from dw_rfm.`b_last_tyear_history_tenants` where part='${tenant}' and stat_date=${hiveconf:thisMonthEnd}) c
	 on a.tenant=c.tenant and a.plat_code = c.plat_code and a.uni_shop_id = c.uni_shop_id and a.uni_id = c.uni_id
	 left join
	 (select * from dw_rfm.`b_before_tyear_history_tenants` where part='${tenant}' and stat_date=${hiveconf:thisMonthEnd}) d
...
(
	...
	from dw_rfm.b_plat_first_buy_history_temp where part='${tenant}' and stat_date=${hiveconf:thisMonthEnd}
)t
