SET mapred.job.name='tenant_shop_history_rfm 租户店铺RFM月底数据分析';
...
CREATE TABLE IF NOT EXISTS dw_rfm.tenant_shop_history_rfm_temp(
	`tenant` string,
    `plat_code` string,
	`uni_shop_id` string,
    `uni_id` string,
...
insert overwrite table dw_rfm.tenant_shop_history_rfm_temp partition(part='${tenant}')
...
-- 1、计算店铺级的RF指标
CREATE TABLE IF NOT EXISTS dw_rfm.`tenant_shop_history_rf_temp`(
	`tenant` string,
    `plat_code` string,
	`uni_shop_id` string,
	`recency` int,
...
-- 店铺级的RF统计临时表
insert overwrite table dw_rfm.`tenant_shop_history_rf_temp` partition(part='${tenant}')
select t.tenant,t.plat_code,t.uni_shop_id,t.recency,t.frequency,t.stat_date,
		...
		from dw_rfm.tenant_shop_history_rfm_temp t
		where t.part='${tenant}'
group by t.tenant,t.plat_code,t.uni_shop_id,t.recency,t.frequency,t.stat_date;

-- 店铺级的RF+两个维度合计的数据
CREATE TABLE IF NOT EXISTS dw_rfm.`tenant_shop_history_rf_result`(
	`tenant` string,
    `plat_code` string,
	`uni_shop_id` string,
	`recency` int,
...
insert overwrite table dw_rfm.tenant_shop_history_rf_result partition(part='${tenant}')
...
from (
    select t.tenant,t.plat_code,t.uni_shop_id,t.recency,t.frequency,t.stat_date,t.customer_num,t.total_payment,t.total_times,t.total_num 
	from dw_rfm.tenant_shop_history_rf_temp t where t.part='${tenant}'
    union all
    ...
    from dw_rfm.tenant_shop_history_rf_temp t1 where t1.part='${tenant}'
    ...
    from dw_rfm.tenant_shop_history_rf_temp t2 where t2.part='${tenant}'
    ...
    from dw_rfm.tenant_shop_history_rf_temp t3 where t3.part='${tenant}'
...
-- 计算RF中客户占比，数据入目标表
insert into table dw_rfm.tenant_active_customer_rf_history partition(part='${tenant}',levels='shop')
...
-- 2、计算店铺级的RM指标
-- 创建活跃客户的RM指标表
CREATE TABLE IF NOT EXISTS dw_rfm.`tenant_shop_history_rm_temp`(
	`tenant` string,
    `plat_code` string,
	`uni_shop_id` string,
	`recency` int,
...
-- 创建店铺的RM临时表
insert overwrite table dw_rfm.tenant_shop_history_rm_temp partition(part='${tenant}')
...
-- 创建RM+合计列的临时结果
CREATE TABLE IF NOT EXISTS dw_rfm.`tenant_shop_history_rm_result`(
	`tenant` string,
    `plat_code` string,
	`uni_shop_id` string,
	`recency` int,
...
insert overwrite table dw_rfm.tenant_shop_history_rm_result partition(part='${tenant}')
select r.tenant,r.plat_code,r.uni_shop_id,r.recency,r.monetary,r.stat_date,r.interval_type,
	...
	r.total_payment/r.total_num as avg_item_pay
from (
    select t.tenant,t.plat_code,t.uni_shop_id,t.recency,t.monetary,t.interval_type,t.stat_date,t.customer_num,t.total_payment,t.total_times,t.total_num 
	from dw_rfm.tenant_shop_history_rm_temp t where t.part='${tenant}'
    union all
    ...
    from dw_rfm.tenant_shop_history_rm_temp t1  where t1.part='${tenant}'
    ...
    from dw_rfm.tenant_shop_history_rm_temp t2  where t2.part='${tenant}'
    ...
    from dw_rfm.tenant_shop_history_rm_temp t3  where t3.part='${tenant}'
...
insert overwrite table dw_rfm.tenant_active_customer_rm_history partition(part='${tenant}',levels='shop')
...
-- 删除中间临时表
ALTER TABLE dw_rfm.tenant_shop_history_rf_temp DROP IF EXISTS PARTITION (part='${tenant}');
ALTER TABLE dw_rfm.tenant_shop_history_rf_result DROP IF EXISTS PARTITION (part='${tenant}');
ALTER TABLE dw_rfm.tenant_shop_history_rm_temp DROP IF EXISTS PARTITION (part='${tenant}');
ALTER TABLE dw_rfm.tenant_shop_history_rm_result DROP IF EXISTS PARTITION (part='${tenant}');
ALTER TABLE dw_rfm.tenant_shop_history_rfm_temp DROP IF EXISTS PARTITION (part='${tenant}');
