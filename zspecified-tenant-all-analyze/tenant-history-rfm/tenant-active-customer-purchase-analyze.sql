...
-- 创建复购客户购买间隔分析结果表
CREATE TABLE IF NOT EXISTS dw_rfm.`tenant_purchase_interval_history`(
	`tenant` string,
    `plat_code` string,
    `uni_shop_id` string,
	`customer_type` int,
...
from(
	...
	from(
		...
		where t.part='${tenant}' 
			and stat_date in(${hiveconf:pre1MonthEnd},${hiveconf:pre2MonthEnd},${hiveconf:pre3MonthEnd},${hiveconf:pre4MonthEnd},
...
select re.tenant,re.plat_code,re.uni_shop_id,re.customer_type,re.interval_days,
	   ...
	   re.stat_date,
	   re.modified
	from dw_rfm.tenant_purchase_interval_history re
	where re.part='${tenant}';
	
-- 需要对tenant_purchase_interval_history下租户分区的数据导出到业务库
