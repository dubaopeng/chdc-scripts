SET mapred.job.name='tenant_plat_rfm_history 租户平台级RFM历史月底数据分析';
...
-- 注:以下数据依赖于平台级客户RFM宽表 b_qqd_plat_rfm
-- 设置任务提交时间
...
-- 创建临时表
CREATE TABLE IF NOT EXISTS dw_rfm.tenant_plat_history_rfm_temp(
	`tenant` string,
    `plat_code` string,
    `uni_id` string,
	`year_payment` double,
...
insert overwrite table dw_rfm.tenant_plat_history_rfm_temp partition(part='${tenant}')
select t.tenant,t.plat_code,t.uni_id,t.year_payment,t.year_buy_times,t.year_buy_num,t.stat_date,
	--R维度打标
...
-- 创建平台级的RF临时表
CREATE TABLE IF NOT EXISTS dw_rfm.`tenant_plat_history_rf_temp`(
	`tenant` string,
    `plat_code` string,
	`recency` int,
	`frequency` int,
...
-- 平台级的RF统计临时表
insert overwrite table dw_rfm.`tenant_plat_history_rf_temp` partition(part='${tenant}')
select t.tenant,t.plat_code,t.recency,t.frequency,t.stat_date,
	count(t.uni_id) customer_num,
	sum(t.year_payment) total_payment,
	sum(t.year_buy_times) total_times,
	sum(t.year_buy_num) total_num
	from dw_rfm.tenant_plat_history_rfm_temp t
	where t.part='${tenant}'
group by t.tenant,t.plat_code,t.recency,t.frequency,t.stat_date;

-- 创建平台级的RF临时结果级
CREATE TABLE IF NOT EXISTS dw_rfm.`tenant_plat_history_rf_result`(
	`tenant` string,
    `plat_code` string,
	`recency` int,
	`frequency` int,
...
-- 租户级的RF+两个维度合计的数据
insert overwrite table dw_rfm.tenant_plat_history_rf_result partition(part='${tenant}')
select r.tenant,r.plat_code,r.recency,r.frequency,r.stat_date,r.customer_num,
...
from (
    select t.tenant,t.plat_code,t.recency,t.frequency,t.stat_date,t.customer_num,t.total_payment,t.total_times,t.total_num 
	from dw_rfm.tenant_plat_history_rf_temp t where t.part='${tenant}'
    union all
    select t1.tenant,t1.plat_code,99 as recency,t1.frequency,t1.stat_date,sum(t1.customer_num) customer_num,sum(t1.total_payment) total_payment,sum(t1.total_times) total_times,sum(t1.total_num) total_num
    from dw_rfm.tenant_plat_history_rf_temp t1 where t1.part='${tenant}'
    group by t1.tenant,t1.plat_code,t1.frequency,t1.stat_date
    union all
    select t2.tenant,t2.plat_code,t2.recency,99 as frequency,t2.stat_date,sum(t2.customer_num) customer_num,sum(t2.total_payment) total_payment,sum(t2.total_times) total_times,sum(t2.total_num) total_num
    from dw_rfm.tenant_plat_history_rf_temp t2 where t2.part='${tenant}'
    group by t2.tenant,t2.plat_code,t2.recency,t2.stat_date
    union all 
    select t3.tenant,t3.plat_code,99 as recency,99 as frequency,t3.stat_date,sum(t3.customer_num) customer_num,sum(t3.total_payment) total_payment ,sum(t3.total_times) total_times,sum(t3.total_num)total_num
    from dw_rfm.tenant_plat_history_rf_temp t3 where t3.part='${tenant}'
    group by t3.tenant,t3.plat_code,t3.stat_date
) r;
...
-- 计算RF中客户占比，数据入目标表
insert overwrite table dw_rfm.tenant_active_customer_rf_history partition(part='${tenant}',levels='plat')
select r.tenant,r.plat_code,null as uni_shop_id,r.recency,r.frequency,r.customer_num,
...
(
	...
	from (
		select c.* from dw_rfm.tenant_plat_history_rf_result c where c.part='${tenant}'
	) t
    left outer join
    (select tenant,plat_code,customer_num,stat_date 
		from dw_rfm.tenant_plat_history_rf_result 
		where part='${tenant}' and recency =99 and frequency=99
	) b
	 on t.tenant = b.tenant and t.plat_code=b.plat_code and t.stat_date=b.stat_date
) r;

-- 2、计算租户级的RM指标
-- 创建活跃客户的RM指标表
CREATE TABLE IF NOT EXISTS dw_rfm.`tenant_plat_history_rm_temp`(
	`tenant` string,
    `plat_code` string,
	`recency` int,
	`monetary` int,
...
-- 创建租户的RM临时表
insert overwrite table dw_rfm.tenant_plat_history_rm_temp partition(part='${tenant}')
select t.tenant,t.plat_code,t.recency,t.monetary1 as monetary,t.stat_date,
	count(t.uni_id) customer_num,
	sum(t.year_payment) total_payment,
	sum(t.year_buy_times) total_times,
	sum(t.year_buy_num) total_num,
	...
	from dw_rfm.tenant_plat_history_rfm_temp  t
	where t.part='${tenant}'
group by t.tenant,t.plat_code,t.recency,t.monetary1,t.stat_date
union all
select t.tenant,t.plat_code,t.recency,t.monetary2 as monetary,t.stat_date,
	count(t.uni_id) customer_num,
	sum(t.year_payment) total_payment,
	sum(t.year_buy_times) total_times,
	sum(t.year_buy_num) total_num,
	...
	from dw_rfm.tenant_plat_history_rfm_temp  t
	where t.part='${tenant}'
group by t.tenant,t.plat_code,t.recency,t.monetary2,t.stat_date
union all
select t.tenant,t.plat_code,t.recency,t.monetary3 as monetary,t.stat_date,
	count(t.uni_id) customer_num,
	sum(t.year_payment) total_payment,
	sum(t.year_buy_times) total_times,
	sum(t.year_buy_num) total_num,
	...
	from dw_rfm.tenant_plat_history_rfm_temp  t
	where t.part='${tenant}'
group by t.tenant,t.plat_code,t.recency,t.monetary3,t.stat_date
union all
select t.tenant,t.plat_code,t.recency,t.monetary4 as monetary,t.stat_date,
	count(t.uni_id) customer_num,
	sum(t.year_payment) total_payment,
	sum(t.year_buy_times) total_times,
	sum(t.year_buy_num) total_num,
	...
	from dw_rfm.tenant_plat_history_rfm_temp  t
	where t.part='${tenant}'
group by t.tenant,t.plat_code,t.recency,t.monetary4,t.stat_date
union all
select t.tenant,t.plat_code,t.recency,t.monetary5 as monetary,t.stat_date,
	count(t.uni_id) customer_num,
	sum(t.year_payment) total_payment,
	sum(t.year_buy_times) total_times,
	sum(t.year_buy_num) total_num,
	...
	from dw_rfm.tenant_plat_history_rfm_temp  t
	where t.part='${tenant}'
group by t.tenant,t.plat_code,t.recency,t.monetary5,t.stat_date
union all
select t.tenant,t.plat_code,t.recency,t.monetary6 as monetary,t.stat_date,
	count(t.uni_id) customer_num,
	sum(t.year_payment) total_payment,
	sum(t.year_buy_times) total_times,
	sum(t.year_buy_num) total_num,
	...
	from dw_rfm.tenant_plat_history_rfm_temp  t
	where t.part='${tenant}'
group by t.tenant,t.plat_code,t.recency,t.monetary6,t.stat_date;


-- 创建RM+合计列的临时结果
CREATE TABLE IF NOT EXISTS dw_rfm.`tenant_plat_history_rm_result`(
	`tenant` string,
    `plat_code` string,
	`recency` int,
	`monetary` int,
...
insert overwrite table dw_rfm.tenant_plat_history_rm_result partition(part='${tenant}')
select r.tenant,r.plat_code,r.recency,r.monetary,r.stat_date,r.interval_type,
	r.customer_num,
	r.total_payment/r.customer_num as avg_payment,
	r.total_payment/r.total_times as avg_guest_pay,
	r.total_num/r.total_times as avg_guest_item,
	r.total_payment/r.total_num as avg_item_pay
from (
    select t.tenant,t.plat_code,t.recency,t.monetary,t.stat_date,t.interval_type,t.customer_num,t.total_payment,t.total_times,t.total_num 
	from dw_rfm.tenant_plat_history_rm_temp t where t.part='${tenant}'
    union all
    select t1.tenant,t1.plat_code,99 as recency,t1.monetary,t1.stat_date,t1.interval_type,sum(t1.customer_num) customer_num,sum(t1.total_payment) total_payment,sum(t1.total_times) total_times,sum(t1.total_num) total_num
    from dw_rfm.tenant_plat_history_rm_temp t1 where t1.part='${tenant}'
    group by t1.tenant,t1.plat_code,t1.monetary,t1.stat_date,t1.interval_type
    union all
    select t2.tenant,t2.plat_code,t2.recency,99 as monetary,t2.stat_date,t2.interval_type,sum(t2.customer_num) customer_num,sum(t2.total_payment) total_payment,sum(t2.total_times) total_times,sum(t2.total_num) total_num
    from dw_rfm.tenant_plat_history_rm_temp t2 where t2.part='${tenant}'
    group by t2.tenant,t2.plat_code,t2.recency,t2.stat_date,t2.interval_type
    union all 
    select t3.tenant,t3.plat_code,99 as recency,99 as monetary,t3.stat_date,t3.interval_type,sum(t3.customer_num) customer_num,sum(t3.total_payment) total_payment ,sum(t3.total_times) total_times,sum(t3.total_num)total_num
    from dw_rfm.tenant_plat_history_rm_temp t3 where t3.part='${tenant}'
    group by t3.tenant,t3.plat_code,t3.stat_date,t3.interval_type
) r;
...
insert overwrite table dw_rfm.tenant_active_customer_rm_history partition(part='${tenant}',levels='plat')
select r.tenant,r.plat_code,null as uni_shop_id,r.recency,r.monetary,r.customer_num,
	case r.bcusnum when 0 then -1 else r.customer_num/r.bcusnum end as customer_rate,
	r.avg_payment,r.avg_guest_pay,r.avg_guest_item,r.avg_item_pay,
	r.interval_type,
	2 as type,
...
(
	...
	case when b.customer_num is null then 0 else b.customer_num end as bcusnum
	from (
		select c.* from dw_rfm.tenant_plat_history_rm_result c where c.part='${tenant}'
	) t
    left outer join
    (select tenant,plat_code,customer_num,stat_date,interval_type from dw_rfm.tenant_plat_history_rm_result 
		where part='${tenant}' and recency =99 and monetary=99
	)  b
    on t.tenant = b.tenant and t.plat_code=b.plat_code and t.stat_date=b.stat_date and t.interval_type=b.interval_type
) r;

-- 删除平台级的RF临时表
ALTER TABLE dw_rfm.tenant_plat_history_rf_temp DROP IF EXISTS PARTITION (part='${tenant}');
ALTER TABLE dw_rfm.tenant_plat_history_rf_result DROP IF EXISTS PARTITION (part='${tenant}');
ALTER TABLE dw_rfm.tenant_plat_history_rm_temp DROP IF EXISTS PARTITION (part='${tenant}');
ALTER TABLE dw_rfm.tenant_plat_history_rm_result DROP IF EXISTS PARTITION (part='${tenant}');
ALTER TABLE dw_rfm.tenant_plat_history_rfm_temp DROP IF EXISTS PARTITION (part='${tenant}');
