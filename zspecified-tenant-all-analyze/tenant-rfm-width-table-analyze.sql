...
insert overwrite table dw_rfm.b_first_buy_history_tenants partition(part = '${tenant}',stat_date='${stat_date}')
...
from(
	...
	from(
		...
		where part='${tenant}'
		and substr(created,1,10) <= '${stat_date}'
	) t where t.rank <= 2
) r
...
insert overwrite table dw_rfm.`b_last_year_history_tenants` partition(part = '${tenant}',stat_date='${stat_date}')
select
	tenant,
    plat_code,
	uni_shop_id,
...
insert overwrite table dw_rfm.`b_last_tyear_history_tenants` partition(part = '${tenant}',stat_date='${stat_date}')
select
	tenant,
    plat_code,
	uni_shop_id,
...
insert overwrite table dw_rfm.`b_before_tyear_history_tenants` partition(part = '${tenant}',stat_date='${stat_date}')
select
	tenant,
    plat_code,
	uni_shop_id,
...
where part='${tenant}'
	and created is not NULL 
	and created < date_add(add_months('${stat_date}',-24),1)
group by
	tenant,
    plat_code,
	uni_shop_id,
...
-- 店铺级RFM指标合并
insert overwrite table dw_rfm.`b_qqd_shop_rfm_tenants` partition(part='${tenant}',stat_date='${stat_date}')
select t.tenant,t.plat_code,t.uni_shop_id,t.uni_id,
...
(
	...
	from 
	 (select * from dw_rfm.`b_first_buy_history_tenants` where part='${tenant}' and stat_date='${stat_date}') a
	 left join
	 (select * from  dw_rfm.`b_last_year_history_tenants` where part='${tenant}' and stat_date='${stat_date}') b
	 on a.tenant=b.tenant and a.plat_code = b.plat_code and a.uni_shop_id = b.uni_shop_id and a.uni_id = b.uni_id
	 left join
	 (select * from dw_rfm.`b_last_tyear_history_tenants` where part='${tenant}' and stat_date='${stat_date}') c
	 on a.tenant=c.tenant and a.plat_code = c.plat_code and a.uni_shop_id = c.uni_shop_id and a.uni_id = c.uni_id
	 left join
	 (select * from dw_rfm.`b_before_tyear_history_tenants` where part='${tenant}' and stat_date='${stat_date}') d
...
insert overwrite table dw_rfm.b_plat_first_buy_history_temp partition(part='${tenant}',stat_date='${stat_date}')
...
insert overwrite table dw_rfm.`b_qqd_plat_rfm_tenants` partition(part='${tenant}',stat_date='${stat_date}')
select r.tenant,r.plat_code,r.uni_id,
	r.earliest_time,
...
from (
	...
	where a.part='${tenant}' and a.stat_date='${stat_date}'
...
insert overwrite table dw_rfm.b_tenant_first_buy_history_temp partition(part='${tenant}',stat_date='${stat_date}')
...
insert overwrite table dw_rfm.b_qqd_tenant_rfm_tenants partition(part='${tenant}',stat_date='${stat_date}')
select r.tenant,r.uni_id,
	r.earliest_time,
	t.first_buy_time,t.first_payment,t.second_buy_time,
	r.year_payment,r.year_buy_times,r.year_buy_num,r.year_first_time,r.year_last_time,
