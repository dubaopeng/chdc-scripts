SET mapred.job.name='tenant-member-point-grade-result-union';
...
CREATE TABLE IF NOT EXISTS dw_rfm.`cix_online_member_point_grade_change_tenants`(
	`card_plan_id` string,
	`rangetype` int,
...
insert overwrite table dw_rfm.cix_online_member_point_grade_change_tenants partition(part='${tenant}')
select t.card_plan_id,t.rangetype,t.effect_points,t.point_change_nums,t.send_points,t.consume_points,
		t.overdue_points,t.newmembers,t.upnums,t.downnums,t.stat_date,${hiveconf:submitTime} as modified
from dw_rfm.b_member_point_grade_change_tenants t
...
CREATE TABLE IF NOT EXISTS dw_rfm.`cix_online_member_grade_transform_tenants`(
	`card_plan_id` string,
	`nowgrade` int, 
	`oldgrade` int, 
	`members` bigint,
...
insert overwrite table dw_rfm.cix_online_member_grade_transform_tenants partition(part='${tenant}')
select t.card_plan_id,t.nowgrade,t.oldgrade,t.members,t.rangetype,t.stat_date,${hiveconf:submitTime} as modified
from dw_rfm.b_member_grade_transform_tenants t
where  t.part='${tenant}' and t.stat_date in(${hiveconf:pre1MonthEnd},${hiveconf:pre2MonthEnd},${hiveconf:pre3MonthEnd},${hiveconf:pre4MonthEnd},
			${hiveconf:pre5MonthEnd},${hiveconf:pre6MonthEnd},${hiveconf:pre7MonthEnd},${hiveconf:pre8MonthEnd},${hiveconf:pre9MonthEnd},
			${hiveconf:pre10MonthEnd},${hiveconf:pre11MonthEnd},${hiveconf:pre12MonthEnd},${hiveconf:pre13MonthEnd});
	
-- 需要将cix_online_member_point_grade_change_tenants通过sqoop同步到业务库中
-- 需要将cix_online_member_grade_transform_tenants通过sqoop同步到业务库中
