SET mapred.job.name='tenant-member-consume-transform-monthend-analyze-历史月底会员消费转化分析';
...
-- 会员消费转化表定义
CREATE TABLE IF NOT EXISTS dw_rfm.`b_last14month_member_monthend_rfm_tenants`(
	`card_plan_id` string,
	`member_id` string,
	`joinmonth` string,
	`uni_id` string,
...
PARTITIONED BY(`tenant` string,`stat_date` string)
ROW FORMAT DELIMITED FIELDS TERMINATED BY '\001' LINES TERMINATED BY '\n'
STORED AS ORC tblproperties ("orc.compress" = "SNAPPY");

insert overwrite table dw_rfm.`b_last14month_member_monthend_rfm_tenants` partition(tenant='${tenant}',stat_date)
...
from 
(
	select '${tenant}' as tenant,r.card_plan_id,r.member_id,r.joinmonth,r.plat_code,
...
left outer join (
	...
	from dw_rfm.b_qqd_shop_rfm_tenants 
	where part='${tenant}' and stat_date =${hiveconf:thisMonthEnd}
)rfm
on tmp.plat_code = rfm.plat_code and tmp.uni_shop_id=rfm.uni_shop_id and tmp.uni_id=rfm.uni_id
where rfm.tenant is not null;

-- 会员消费转化表定义
CREATE TABLE IF NOT EXISTS dw_rfm.`b_member_consume_transform_tenants`(
	`plat_code` string,
    `uni_shop_id` string,
	`card_plan_id` string,
...
insert overwrite table dw_rfm.b_member_consume_transform_tenants partition(tenant='${tenant}',stat_date)
select r1.plat_code,r1.uni_shop_id,r1.card_plan_id,
...
(
	...
	select a.plat_code,a.uni_shop_id,a.card_plan_id,a.joinmonth,
		   count(distinct a.member_id) totalmembers,
		   sum(a.payment) payments
	from dw_rfm.b_last14month_member_monthend_rfm_tenants a 
		where a.tenant='${tenant}' and a.stat_date=${hiveconf:thisMonthEnd}
	group by a.plat_code,a.uni_shop_id,a.card_plan_id,a.joinmonth
) r1
left join
(
	-- 计算frequency对应人数
	select t.plat_code,t.uni_shop_id,t.card_plan_id,t.joinmonth,
	...
	from
	(
		select a.plat_code,a.uni_shop_id,a.card_plan_id,a.joinmonth,a.frequency,
			   count(distinct a.member_id) fmembers
		from dw_rfm.b_last14month_member_monthend_rfm_tenants a 
		where a.tenant='${tenant}' and a.stat_date=${hiveconf:thisMonthEnd}
		group by a.plat_code,a.uni_shop_id,a.card_plan_id,a.joinmonth,a.frequency
	) t
	group by t.plat_code,t.uni_shop_id,t.card_plan_id,t.joinmonth
) r2
on r1.plat_code=r2.plat_code and r1.uni_shop_id=r2.uni_shop_id
   and r1.card_plan_id=r2.card_plan_id and r1.joinmonth=r2.joinmonth;

--计算店铺+卡的合计行数据
insert into table dw_rfm.b_member_consume_transform_tenants partition(tenant='${tenant}',stat_date)
select t.plat_code,t.uni_shop_id,t.card_plan_id,'all' as join_month,
	...
	${hiveconf:isMonthEnd} as end_month,
	${hiveconf:thisMonthEnd} as stat_date
from dw_rfm.b_member_consume_transform_tenants t
where t.tenant='${tenant}' and t.stat_date=${hiveconf:thisMonthEnd}
group by t.plat_code,t.uni_shop_id,t.card_plan_id;


-- 基于店铺级的数据，计算平台级数据 和 租户级数据
insert into table dw_rfm.b_member_consume_transform_tenants partition(tenant='${tenant}',stat_date)
select t.plat_code,null as uni_shop_id,t.card_plan_id,t.join_month,
	sum(t.members) as members,
	sum(t.payments) as payments,
	sum(t.f0) as f0,
	sum(t.f1) as f1,
...
from dw_rfm.b_member_consume_transform_tenants t 
where t.tenant='${tenant}' and t.stat_date=${hiveconf:thisMonthEnd}
group by t.plat_code,t.card_plan_id,t.join_month;


insert into table dw_rfm.b_member_consume_transform_tenants partition(tenant='${tenant}',stat_date)
select null as plat_code,null as uni_shop_id,t1.card_plan_id,t1.join_month,
	...
	${hiveconf:isMonthEnd} as end_month,
	${hiveconf:thisMonthEnd} as stat_date
from dw_rfm.b_member_consume_transform_tenants t1 
where t1.tenant='${tenant}' and t1.stat_date=${hiveconf:thisMonthEnd}
group by t1.card_plan_id,t1.join_month;

--各月份数据完成后，需要将各分区的数据合并到一张表中,同步给业务库
-- 统计完成后需要将表 dw_rfm.b_member_consume_transform_tenants 各个分区下的数据合并到同一个表中同步给业务库
