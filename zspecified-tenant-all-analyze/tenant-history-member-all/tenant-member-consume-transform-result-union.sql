SET mapred.job.name='tenant-member-consume-transform-result-union';
...
CREATE TABLE IF NOT EXISTS dw_rfm.`cix_online_member_consume_transform_tenants`(
	`tenant` string,
	`plat_code` string,
    `uni_shop_id` string,
	`card_plan_id` string,
...
insert overwrite table dw_rfm.cix_online_member_consume_transform_tenants partition(part='${tenant}')
select '${tenant}' as tenant,t.plat_code,t.uni_shop_id,t.card_plan_id,t.join_month,t.members,t.payments,
...
where t.tenant='${tenant}' and t.stat_date in(${hiveconf:pre1MonthEnd},${hiveconf:pre2MonthEnd},${hiveconf:pre3MonthEnd},${hiveconf:pre4MonthEnd},
...
-- 需要将cix_online_member_consume_transform_tenants通过sqoop同步到业务库中
