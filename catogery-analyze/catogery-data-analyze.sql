...
-- 商品信息: dw_business.b_std_product (需接入)
-- 店铺类目表：dw_business.b_plat_shop_category (需接入)
...
where (part > add_months('${stat_date}',-25) and part < date_add('${stat_date}',1)) 
...
from (
	...
	from (
		...
		left join dw_business.b_std_product c
	...
	left join
	   -- 平台店铺类目信息表
	   dw_business.b_plat_shop_category r2
...
(
	...
	(select *,row_number() over(partition by plat_code,uni_shop_id order by total_payment desc) as rank 
...
(
	...
	from 
	(
		select r.plat_code,r.uni_shop_id,r.shop_id,r.parent_category_id,sum(r.total_payment) total_payment
		from (
		...
		group by r.plat_code,r.uni_shop_id,r.shop_id,r.parent_category_id
	) t
) re
where re.rank <= 5;
...
create table dw_rfm.b_last_year_shop_category_sel_num as
...
from (
	...
	left join dw_business.b_plat_shop_category r2  -- 平台店铺类目信息表
	on r1.plat_code=r2.plat_code and r1.shop_id=r2.shop_id and r1.category_id=r2.category_id
	where r2.parent_category_id is not null
) d
...
from (
	select a.parent_category_id,a.plat_code,a.uni_shop_id,b.rate,b.new_rate,b.old_rate
	from dw_rfm.b_category_top5_shop a
	join
	(
		select re.tenant,re.plat_code,re.uni_shop_id,
		...
		from (
			select r.tenant,r.plat_code,r.uni_shop_id,
				sum(r.active) as activeNum,
				sum(r.repurchase) as repurNum,
				sum(r.active_new) as activeNewNum,
				sum(r.new_repurchase) as newrepurNum,
				sum(r.active_old) as activeOldNum,
				sum(r.old_repurchase) as oldrepurNum
			from (
...
create table dw_rfm.b_category_retention_temp as
