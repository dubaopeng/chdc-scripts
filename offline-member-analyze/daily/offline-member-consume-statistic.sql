...
insert overwrite table dw_rfm.b_offline_shop_last30day_trade partition(part='${stat_date}')
...
where part='${stat_date}' and plat_code='OFFLINE';
...
-- 计算各个等级在指定时段内的会员消费统计
insert overwrite table dw_rfm.b_opencard_shop_member_consume_statics partition(part='${stat_date}')
...
from(
	...
	where part='${stat_date}' and yestoday=1 and shop_id=open_shop_id
	...
	select card_plan_id,plat_code,shop_id,uni_shop_id,grade,2 as date_type,
		   ...
		   count(distinct concat(member_id,uni_shop_id,substr(created,1,10))) as total_times
	from dw_rfm.b_offline_shop_last30day_trade
	where part='${stat_date}' and thisweek=1 and shop_id=open_shop_id
	group by card_plan_id,plat_code,shop_id,uni_shop_id,grade
	union all 
	select card_plan_id,plat_code,shop_id,uni_shop_id,grade,3 as date_type,
		   ...
		   count(distinct concat(member_id,uni_shop_id,substr(created,1,10))) as total_times
	from dw_rfm.b_offline_shop_last30day_trade
	where part='${stat_date}' and thismonth=1 and shop_id=open_shop_id
	group by card_plan_id,plat_code,shop_id,uni_shop_id,grade
	union all 
	select card_plan_id,plat_code,shop_id,uni_shop_id,grade,4 as date_type,
		   ...
		   count(distinct concat(member_id,uni_shop_id,substr(created,1,10))) as total_times
	from dw_rfm.b_offline_shop_last30day_trade
	where part='${stat_date}' and last7day=1 and shop_id=open_shop_id
	group by card_plan_id,plat_code,shop_id,uni_shop_id,grade
	union all
	select card_plan_id,plat_code,shop_id,uni_shop_id,grade,5 as date_type,
		   ...
		   count(distinct concat(member_id,uni_shop_id,substr(created,1,10))) as total_times
	from dw_rfm.b_offline_shop_last30day_trade
	where part='${stat_date}' and last30day=1 and shop_id=open_shop_id
	group by card_plan_id,plat_code,shop_id,uni_shop_id,grade
)t;

-- 开卡店铺内会员在该店铺消费汇总，不限等级
insert into table dw_rfm.b_opencard_shop_member_consume_statics partition(part='${stat_date}')
...
insert overwrite table dw_rfm.b_shop_all_member_consume_statics partition(part='${stat_date}')
...
--按日期汇总的店铺所有会员消费数据统计
insert into table dw_rfm.b_shop_all_member_consume_statics partition(part='${stat_date}')
...
insert overwrite table dw_rfm.b_offline_shop_members_count_temp partition(part='${stat_date}')
...
-- 不限等级时店铺的会员存量
insert into dw_rfm.b_offline_shop_members_count_temp partition(part='${stat_date}')
...
insert overwrite table dw_rfm.cix_offline_member_consume_statistic partition(part='${stat_date}')
...
from (select card_plan_id,plat_code,uni_shop_id,shop_id,shop_name,grade,members from dw_rfm.b_offline_shop_members_count_temp where part='${stat_date}') a
left join 
...
-- 删除各个临时表的分区
ALTER TABLE dw_rfm.b_offline_shop_last30day_trade DROP IF EXISTS PARTITION (part='${stat_date}');
ALTER TABLE dw_rfm.b_opencard_shop_member_consume_statics DROP IF EXISTS PARTITION (part='${stat_date}');
ALTER TABLE dw_rfm.b_shop_all_member_consume_statics DROP IF EXISTS PARTITION (part='${stat_date}');
ALTER TABLE dw_rfm.b_offline_shop_members_count_temp DROP IF EXISTS PARTITION (part='${stat_date}');
