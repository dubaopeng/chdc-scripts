...
set preMonthFirstDay=date_sub(concat(substr('${stat_date}',0,7),'-01'),1);
set preMonthEndDay=add_months(${hiveconf:preMonthFirstDay},-${monthNum});
...
insert overwrite table dw_rfm.b_offline_shop_last30day_trade partition(part)
select card_plan_id,plat_code,shop_id,uni_shop_id,receive_payment,created,
	   ...
	   ${hiveconf:preMonthEndDay} as part
from dw_rfm.b_offline_last30day_member_trade_temp
where part=${hiveconf:preMonthEndDay} and plat_code='OFFLINE';
...
-- 计算各个等级在指定时段内的会员消费统计
insert overwrite table dw_rfm.b_opencard_shop_member_consume_statics partition(part)
...
from(
	select card_plan_id,plat_code,shop_id,uni_shop_id,grade,1 as date_type,
	   ...
	   count(distinct concat(member_id,uni_shop_id,substr(created,1,10))) as total_times
	from dw_rfm.b_offline_shop_last30day_trade
	where part=${hiveconf:preMonthEndDay} and yestoday=1 and shop_id=open_shop_id
	...
	select card_plan_id,plat_code,shop_id,uni_shop_id,grade,2 as date_type,
		   ...
		   count(distinct concat(member_id,uni_shop_id,substr(created,1,10))) as total_times
	from dw_rfm.b_offline_shop_last30day_trade
	where part=${hiveconf:preMonthEndDay} and thisweek=1 and shop_id=open_shop_id
	group by card_plan_id,plat_code,shop_id,uni_shop_id,grade
	union all 
	select card_plan_id,plat_code,shop_id,uni_shop_id,grade,3 as date_type,
		   ...
		   count(distinct concat(member_id,uni_shop_id,substr(created,1,10))) as total_times
	from dw_rfm.b_offline_shop_last30day_trade
	where part=${hiveconf:preMonthEndDay} and thismonth=1 and shop_id=open_shop_id
	group by card_plan_id,plat_code,shop_id,uni_shop_id,grade
	union all 
	select card_plan_id,plat_code,shop_id,uni_shop_id,grade,4 as date_type,
		   ...
		   count(distinct concat(member_id,uni_shop_id,substr(created,1,10))) as total_times
	from dw_rfm.b_offline_shop_last30day_trade
	where part=${hiveconf:preMonthEndDay} and last7day=1 and shop_id=open_shop_id
	group by card_plan_id,plat_code,shop_id,uni_shop_id,grade
	union all
	select card_plan_id,plat_code,shop_id,uni_shop_id,grade,5 as date_type,
		   ...
		   count(distinct concat(member_id,uni_shop_id,substr(created,1,10))) as total_times
	from dw_rfm.b_offline_shop_last30day_trade
	where part=${hiveconf:preMonthEndDay} and last30day=1 and shop_id=open_shop_id
	group by card_plan_id,plat_code,shop_id,uni_shop_id,grade
)t;

-- 开卡店铺内会员在该店铺消费汇总，不限等级
insert into table dw_rfm.b_opencard_shop_member_consume_statics partition(part)
...
from(
	...
	where part=${hiveconf:preMonthEndDay}
...
insert overwrite table dw_rfm.b_shop_all_member_consume_statics partition(part)
...
from (
	select card_plan_id,plat_code,open_shop_id,grade,1 as date_type,
	   count(distinct member_id) as members,
	   sum(receive_payment) as total_payment,
	   count(distinct concat(member_id,open_shop_id,substr(created,1,10))) as total_times
	from dw_rfm.b_offline_shop_last30day_trade
	where part=${hiveconf:preMonthEndDay} and yestoday=1
	...
	select card_plan_id,plat_code,open_shop_id,grade,2 as date_type,
		   ...
		   count(distinct concat(member_id,open_shop_id,substr(created,1,10))) as total_times
	from dw_rfm.b_offline_shop_last30day_trade
	where part=${hiveconf:preMonthEndDay} and thisweek=1
	group by card_plan_id,plat_code,open_shop_id,grade
	union all 
	select card_plan_id,plat_code,open_shop_id,grade,3 as date_type,
		   ...
		   count(distinct concat(member_id,open_shop_id,substr(created,1,10))) as total_times
	from dw_rfm.b_offline_shop_last30day_trade
	where part=${hiveconf:preMonthEndDay} and thismonth=1
	group by card_plan_id,plat_code,open_shop_id,grade
	union all 
	select card_plan_id,plat_code,open_shop_id,grade,4 as date_type,
		   ...
		   count(distinct concat(member_id,open_shop_id,substr(created,1,10))) as total_times
	from dw_rfm.b_offline_shop_last30day_trade
	where part=${hiveconf:preMonthEndDay} and last7day=1
	group by card_plan_id,plat_code,open_shop_id,grade
	union all
	select card_plan_id,plat_code,open_shop_id,grade,5 as date_type,
		   ...
		   count(distinct concat(member_id,open_shop_id,substr(created,1,10))) as total_times
	from dw_rfm.b_offline_shop_last30day_trade
	where part=${hiveconf:preMonthEndDay} and last30day=1
	group by card_plan_id,plat_code,open_shop_id,grade
) t;

--按日期汇总的店铺所有会员消费数据统计
insert into table dw_rfm.b_shop_all_member_consume_statics partition(part)
...
insert overwrite table dw_rfm.b_offline_shop_members_count_temp partition(part)
select t.card_plan_id,t.plat_code,t.uni_shop_id,t1.shop_id,t1.shop_name,t.grade,t.members,${hiveconf:preMonthEndDay} as part
from (
	select card_plan_id,plat_code,uni_shop_id,grade,count(member_id) as members
	from dw_business.b_std_member_base_info 
	where plat_code='OFFLINE' and created <= ${hiveconf:preMonthEndDay}
...
-- 不限等级时店铺的会员存量
insert into dw_rfm.b_offline_shop_members_count_temp partition(part)
select card_plan_id,plat_code,uni_shop_id,shop_id,shop_name,99 as grade,sum(members) as members,${hiveconf:preMonthEndDay} as part
...
insert overwrite table dw_rfm.cix_offline_member_consume_statistic partition(part)
select a.card_plan_id,a.uni_shop_id,a.shop_name,b.date_type,a.grade,
	   ...
	   ${hiveconf:preMonthEndDay} as stat_date,
	   ${hiveconf:submitTime} as modified,
	   ${hiveconf:preMonthEndDay} as part
from (select card_plan_id,plat_code,uni_shop_id,shop_id,shop_name,grade,members from dw_rfm.b_offline_shop_members_count_temp where part=${hiveconf:preMonthEndDay}) a
left join 
(select card_plan_id,uni_shop_id,date_type,grade,members,total_payment,avg_price,avg_times,single_price 
	from dw_rfm.b_opencard_shop_member_consume_statics
	where part=${hiveconf:preMonthEndDay}
) b
on a.card_plan_id=b.card_plan_id and a.uni_shop_id=b.uni_shop_id and a.grade=b.grade
left join 
(select card_plan_id,plat_code,open_shop_id,grade,date_type,members,total_payment,avg_price,avg_times,single_price
	from dw_rfm.b_shop_all_member_consume_statics
	where part=${hiveconf:preMonthEndDay}
) c
