...
set yestoday=date_sub(${hiveconf:preMonthEndDay},1);
set weekfirst=date_sub(${hiveconf:preMonthEndDay},pmod(datediff(${hiveconf:preMonthEndDay}, concat(year(${hiveconf:preMonthEndDay}),'-01-01'))-6,7));
set monthfirst=date_sub(${hiveconf:preMonthEndDay},dayofmonth(${hiveconf:preMonthEndDay})-1);
set yearfirst=concat(substr(${hiveconf:preMonthEndDay},0,4),'-01-01');
set last7day=date_sub(${hiveconf:preMonthEndDay},7);
set last30day=date_sub(${hiveconf:preMonthEndDay},30);
set lastyear=add_months(${hiveconf:preMonthEndDay},-12);
...
insert overwrite table dw_rfm.b_shop_memeber_created_reconized_temp partition(part)
...
where (plat_code='TAOBAO' or plat_code='OFFLINE') and substr(created,1,10) <= ${hiveconf:preMonthEndDay};
...
insert overwrite table dw_rfm.b_shop_memeber_increm_staticits_temp partition(part)
...
insert overwrite table dw_rfm.cix_offline_shop_member_change_statistic partition(part)
...
from (
	...
	where a.part=${hiveconf:preMonthEndDay}
) t
left join(
