...
-- 只对淘宝和线下平台的数据
insert overwrite table dw_rfm.b_offline_last30day_trade_temp partition(part,plat_code)
...
from (
	...
	left join (
		...
		where
		  part >= substr(date_sub(${hiveconf:preMonthEndDay},30),1,7)
		  and part <= substr(${hiveconf:preMonthEndDay},1,7)
		  and (plat_code='TAOBAO' or plat_code='OFFLINE')
		  and (created is not NULL and substr(created,1,10) > date_sub(${hiveconf:preMonthEndDay},30) and substr(created,1,10) <= ${hiveconf:preMonthEndDay})
...
insert overwrite table dw_rfm.b_offline_last30day_member_trade_temp partition(part,plat_code)
...
from (
	...
	where part=${hiveconf:preMonthEndDay} and member_id is not null
) t
