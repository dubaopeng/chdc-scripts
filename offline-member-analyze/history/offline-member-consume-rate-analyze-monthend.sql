...
-- 计算卡、平台、店铺的总统计指标
insert overwrite table dw_rfm.b_shop_sale_total_statics_temp partition(part)
select t.card_plan_id,t.plat_code,t.shop_id,t.uni_shop_id,t.date_type,t.total_payment,t.total_times,${hiveconf:preMonthEndDay} as part
from(
...
insert overwrite table dw_rfm.b_plat_sale_total_statics_temp partition(part)
...
insert overwrite table dw_rfm.b_shop_member_total_statics_temp partition(part)
select t.card_plan_id,t.plat_code,t.shop_id,t.date_type,t.mtotal_payment,t.mtotal_times,${hiveconf:preMonthEndDay} as part
from(
...
insert overwrite table dw_rfm.b_plat_member_total_statics_temp partition(part)
select card_plan_id,plat_code,date_type,
...
insert overwrite table dw_rfm.b_shop_grade_total_statics_temp partition(part)
select t.card_plan_id,t.plat_code,t.shop_id,t.grade,t.date_type,t.mtotal_payment,t.mtotal_times,${hiveconf:preMonthEndDay} as part
from (
...
-- 计算店铺各个时段的会员销售综合统计数据
insert into table dw_rfm.b_shop_grade_total_statics_temp partition(part)
...
insert overwrite table dw_rfm.b_plat_grade_total_statics_temp partition(part)
...
insert overwrite table dw_rfm.cix_offline_plat_member_sale_rate partition(part)
...
from (
	select a.card_plan_id,a.plat_code,a.date_type,a.total_payment,a.total_times,
		   ...
		   b.grade
	from (select card_plan_id,plat_code,date_type,total_payment,total_times from dw_rfm.b_plat_sale_total_statics_temp where part=${hiveconf:preMonthEndDay})a
	left join 
	(select card_plan_id,plat_code,date_type,mtotal_payment,mtotal_times,grade from dw_rfm.b_plat_grade_total_statics_temp where part=${hiveconf:preMonthEndDay}) b
...
insert overwrite table dw_rfm.cix_offline_sale_shops_of_card partition(part)
...
from(
	...
	from (
		select card_plan_id,plat_code,uni_shop_id,shop_id,date_type,total_payment,total_times 
			from dw_rfm.b_shop_sale_total_statics_temp where part=${hiveconf:preMonthEndDay}) a
	left join 
	(
...
left join 
(select card_plan_id,plat_code,shop_id,date_type,grade,mtotal_payment,mtotal_times from dw_rfm.b_shop_grade_total_statics_temp where part=${hiveconf:preMonthEndDay}) c
