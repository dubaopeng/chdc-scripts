...
-- 店铺会员变化分析结果
drop table if exists dw_rfm.cix_offline_shop_member_change_statistic;
...
insert overwrite table dw_rfm.cix_offline_shop_member_change_statistic
...
-- 设置分析结果日志
drop table if exists dw_rfm.cix_offline_member_change_analysis_log;
CREATE TABLE IF NOT EXISTS dw_rfm.`cix_offline_member_change_analysis_log`(
	`table_name` string,
    `available` string,
    `modified` string,
	`stat_date` string
...
insert overwrite table dw_rfm.cix_offline_member_change_analysis_log 
select a.tablename,1 as available,${hiveconf:submitTime} as modified,'${stat_date}' as stat_date
from ( select 'cix_offline_shop_member_change_statistic' as tablename ) a;
