...
-- 计算各时段店铺存量会员数汇总
drop table if exists dw_rfm.b_shop_members_stroe_temp;
create table if not exists dw_rfm.b_shop_members_stroe_temp(
	card_plan_id string,
	plat_code string,
	uni_shop_id string,
	shop_id string,
...
insert overwrite table dw_rfm.b_shop_members_stroe_temp
select t.card_plan_id,t.plat_code,t.uni_shop_id,t.shop_id,t.shop_name,t1.member_id,t1.grade,
	case when substr(t1.created,1,10) < ${hiveconf:yestoday} then 1 else 0 end as yestoday,
    case when substr(t1.created,1,10) < ${hiveconf:weekfirst} then 1 else 0 end as thisweek,
    case when substr(t1.created,1,10) < ${hiveconf:monthfirst} then 1 else 0 end as thismonth,
    case when substr(t1.created,1,10) < ${hiveconf:last7day} then 1 else 0 end as last7day,
    case when substr(t1.created,1,10) < ${hiveconf:last30day} then 1 else 0 end as last30day
from (
	select a.card_plan_id,a.plat_code,a.uni_shop_id,a.shop_id,b.shop_name
	from (
		select card_plan_id,plat_code,shop_id,uni_shop_id
		from dw_rfm.b_offline_shop_last30day_trade
...
left join
(
	select card_plan_id,plat_code,uni_shop_id,member_id,grade,created
	from dw_business.b_std_member_base_info where plat_code='OFFLINE'
) t1
on t.card_plan_id=t1.card_plan_id and t.plat_code=t1.plat_code and t.uni_shop_id=t1.uni_shop_id;
...
select t.card_plan_id,t.plat_code,t.uni_shop_id,t.shop_id,t.shop_name,t.grade,t.date_type,t.members
from (
	select card_plan_id,plat_code,uni_shop_id,shop_id,shop_name,grade,1 as date_type,count(member_id) as members
	from dw_rfm.b_shop_members_stroe_temp
	where yestoday=1
	group by card_plan_id,plat_code,uni_shop_id,shop_id,shop_name,grade
	union all 
	select card_plan_id,plat_code,uni_shop_id,shop_id,shop_name,grade,2 as date_type,count(member_id) as members
	from dw_rfm.b_shop_members_stroe_temp
	where thisweek=1
	group by card_plan_id,plat_code,uni_shop_id,shop_id,shop_name,grade
	union all 
	select card_plan_id,plat_code,uni_shop_id,shop_id,shop_name,grade,3 as date_type,count(member_id) as members
	from dw_rfm.b_shop_members_stroe_temp
	where thismonth=1
	group by card_plan_id,plat_code,uni_shop_id,shop_id,shop_name,grade
	union all
	select card_plan_id,plat_code,uni_shop_id,shop_id,shop_name,grade,4 as date_type,count(member_id) as members
	from dw_rfm.b_shop_members_stroe_temp
	where last7day=1
	group by card_plan_id,plat_code,uni_shop_id,shop_id,shop_name,grade
	union all
	select card_plan_id,plat_code,uni_shop_id,shop_id,shop_name,grade,5 as date_type,count(member_id) as members
	from dw_rfm.b_shop_members_stroe_temp
	where last30day=1
	group by card_plan_id,plat_code,uni_shop_id,shop_id,shop_name,grade
)t;
...
select card_plan_id,plat_code,uni_shop_id,shop_id,shop_name,99 as grade,date_type,sum(members) as members
from dw_rfm.b_offline_shop_members_count_temp
group by card_plan_id,plat_code,uni_shop_id,shop_id,shop_name,date_type;
...
select a.card_plan_id,a.uni_shop_id,a.shop_name,a.date_type,a.grade,
...
on a.card_plan_id=b.card_plan_id and a.uni_shop_id=b.uni_shop_id and a.date_type=b.date_type and a.grade=b.grade
left join dw_rfm.b_shop_all_member_consume_statics c
on a.card_plan_id=c.card_plan_id and a.plat_code=c.plat_code and a.shop_id=c.open_shop_id and a.date_type=c.date_type and a.grade=c.grade;
