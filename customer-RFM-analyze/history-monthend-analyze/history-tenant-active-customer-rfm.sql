...
-- 租户级的RF统计临时表
drop table if exists dw_rfm.b_tenant_history_rf_temp;
create table dw_rfm.b_tenant_history_rf_temp as
select t.tenant,t.recency,t.frequency,t.stat_date,
	...
	from dw_rfm.b_tenant_history_rfm_temp t
group by t.tenant,t.recency,t.frequency,t.stat_date;

-- 租户级的RF+两个维度合计的数据
drop table if exists dw_rfm.b_tenant_history_rf_result;
create table dw_rfm.b_tenant_history_rf_result as
...
from (
    select t.tenant,t.recency,t.frequency,t.stat_date,t.customer_num,t.total_payment,t.total_times,t.total_num from dw_rfm.b_tenant_history_rf_temp t
    union all
    ...
    from dw_rfm.b_tenant_history_rf_temp t1
    ...
    from dw_rfm.b_tenant_history_rf_temp t2
    ...
    from dw_rfm.b_tenant_history_rf_temp t3
    group by t3.tenant,t3.stat_date
) r;

-- 计算RF中客户占比，数据入目标表
insert overwrite table dw_rfm.b_active_customer_rf_history partition(part='tenant')
...
(
    ...
    from dw_rfm.b_tenant_history_rf_result t
    left outer join
	(select tenant,customer_num,stat_date from dw_rfm.b_tenant_history_rf_result where recency =99 and frequency=99) b
...
-- 创建RM+合计列的临时结果
drop table if exists dw_rfm.b_tenant_history_rm_result;
create table dw_rfm.b_tenant_history_rm_result as
select r.tenant,r.recency,r.monetary,r.stat_date,r.interval_type,
	...
	r.total_payment/r.total_num as avg_item_pay
from (
    select t.tenant,t.recency,t.monetary,t.stat_date,t.interval_type,t.customer_num,t.total_payment,t.total_times,t.total_num from dw_rfm.b_tenant_history_rm_temp t
    union all
    ...
    from dw_rfm.b_tenant_history_rm_temp t1
    ...
    from dw_rfm.b_tenant_history_rm_temp t2
    ...
    from dw_rfm.b_tenant_history_rm_temp t3
    group by t3.tenant,t3.stat_date,t3.interval_type
) r;

-- 计算RM中客户占比，数据入目标表
insert overwrite table dw_rfm.b_active_customer_rm_history partition(part='tenant')
...
(
    select t.*,
	case when b.customer_num is null then 0 else b.customer_num end as bcusnum
    from dw_rfm.b_tenant_history_rm_result t
    left outer join
	(select tenant,customer_num,stat_date,interval_type from dw_rfm.b_tenant_history_rm_result where recency =99 and monetary=99) b
