SET mapred.job.name='b_tenant_rfm_temp-租户级RFM数据打标';
...
drop table if exists dw_rfm.b_tenant_rfm_temp;

create table dw_rfm.b_tenant_rfm_temp as
select t.tenant,t.uni_id,t.year_payment,t.year_buy_times,t.year_buy_num,
--R维度打标
...
-- 租户级的RF统计临时表
drop table if exists dw_rfm.b_tenant_rf_temp;
create table dw_rfm.b_tenant_rf_temp as
select t.tenant,t.recency,t.frequency,
	count(t.uni_id) customer_num,
	sum(t.year_payment) total_payment,
	sum(t.year_buy_times) total_times,
	sum(t.year_buy_num) total_num
	from dw_rfm.b_tenant_rfm_temp t
group by t.tenant,t.recency,t.frequency;

-- 租户级的RF+两个维度合计的数据
drop table if exists dw_rfm.b_tenant_rf_result;
create table dw_rfm.b_tenant_rf_result as
select r.tenant,r.recency,r.frequency,r.customer_num,
...
from (
    select t.tenant,t.recency,t.frequency,t.customer_num,t.total_payment,t.total_times,t.total_num from dw_rfm.b_tenant_rf_temp t
    union all
    select t1.tenant,99 as recency,t1.frequency,sum(t1.customer_num) customer_num,sum(t1.total_payment) total_payment,sum(t1.total_times) total_times,sum(t1.total_num) total_num
    from dw_rfm.b_tenant_rf_temp t1
    group by t1.tenant,t1.frequency
    union all
    select t2.tenant,t2.recency,99 as frequency,sum(t2.customer_num) customer_num,sum(t2.total_payment) total_payment,sum(t2.total_times) total_times,sum(t2.total_num) total_num
    from dw_rfm.b_tenant_rf_temp t2
    group by t2.tenant,t2.recency
    union all 
    select t3.tenant,99 as recency,99 as frequency,sum(t3.customer_num) customer_num,sum(t3.total_payment) total_payment ,sum(t3.total_times) total_times,sum(t3.total_num)total_num
    from dw_rfm.b_tenant_rf_temp t3
    group by t3.tenant
) r;

-- 计算RF中客户占比，数据入目标表
insert overwrite table dw_rfm.b_active_customer_rf_temp partition(part='${stat_date}',level='tenant')
...
(
    ...
    from dw_rfm.b_tenant_rf_result t
    left outer join
	(select tenant,customer_num from dw_rfm.b_tenant_rf_result where recency =99 and frequency=99) b
    on t.tenant = b.tenant 
) r;
...
-- 创建租户的RM临时表
drop table if exists dw_rfm.b_tenant_rm_temp;
create table dw_rfm.b_tenant_rm_temp as
select t.tenant,t.recency,t.monetary1 as monetary,
	count(t.uni_id) customer_num,
	sum(t.year_payment) total_payment,
	sum(t.year_buy_times) total_times,
	sum(t.year_buy_num) total_num,
	1 as interval_type
	from dw_rfm.b_tenant_rfm_temp t
	group by t.tenant,t.recency,t.monetary1
union all
select t.tenant,t.recency,t.monetary2 as monetary,
	count(t.uni_id) customer_num,
	sum(t.year_payment) total_payment,
	sum(t.year_buy_times) total_times,
	sum(t.year_buy_num) total_num,
	2 as interval_type
	from dw_rfm.b_tenant_rfm_temp t
	group by t.tenant,t.recency,t.monetary2
union all
select t.tenant,t.recency,t.monetary3 as monetary,
	count(t.uni_id) customer_num,
	sum(t.year_payment) total_payment,
	sum(t.year_buy_times) total_times,
	sum(t.year_buy_num) total_num,
	3 as interval_type
	from dw_rfm.b_tenant_rfm_temp t
	group by t.tenant,t.recency,t.monetary3
union all
select t.tenant,t.recency,t.monetary4 as monetary,
	count(t.uni_id) customer_num,
	sum(t.year_payment) total_payment,
	sum(t.year_buy_times) total_times,
	sum(t.year_buy_num) total_num,
	4 as interval_type
	from dw_rfm.b_tenant_rfm_temp t
	group by t.tenant,t.recency,t.monetary4
union all
select t.tenant,t.recency,t.monetary5 as monetary,
	count(t.uni_id) customer_num,
	sum(t.year_payment) total_payment,
	sum(t.year_buy_times) total_times,
	sum(t.year_buy_num) total_num,
	5 as interval_type
	from dw_rfm.b_tenant_rfm_temp t
	group by t.tenant,t.recency,t.monetary5
union all
select t.tenant,t.recency,t.monetary6 as monetary,
	count(t.uni_id) customer_num,
	sum(t.year_payment) total_payment,
	sum(t.year_buy_times) total_times,
	sum(t.year_buy_num) total_num,
	6 as interval_type
	from dw_rfm.b_tenant_rfm_temp t
	group by t.tenant,t.recency,t.monetary6;

-- 创建RM+合计列的临时结果
drop table if exists dw_rfm.b_tenant_rm_result;
create table dw_rfm.b_tenant_rm_result as
select r.tenant,r.recency,r.monetary,r.interval_type,
	r.customer_num,
	r.total_payment/r.customer_num as avg_payment,
	r.total_payment/r.total_times as avg_guest_pay,
	r.total_num/r.total_times as avg_guest_item,
	r.total_payment/r.total_num as avg_item_pay
from (
    select t.tenant,t.recency,t.monetary,t.interval_type,t.customer_num,t.total_payment,t.total_times,t.total_num from dw_rfm.b_tenant_rm_temp t
    union all
    select t1.tenant,99 as recency,t1.monetary,t1.interval_type,sum(t1.customer_num) customer_num,sum(t1.total_payment) total_payment,sum(t1.total_times) total_times,sum(t1.total_num) total_num
    from dw_rfm.b_tenant_rm_temp t1
    group by t1.tenant,t1.monetary,t1.interval_type
    union all
    select t2.tenant,t2.recency,99 as monetary,t2.interval_type,sum(t2.customer_num) customer_num,sum(t2.total_payment) total_payment,sum(t2.total_times) total_times,sum(t2.total_num) total_num
    from dw_rfm.b_tenant_rm_temp t2
    group by t2.tenant,t2.recency,t2.interval_type
    union all 
    select t3.tenant,99 as recency,99 as monetary,t3.interval_type,sum(t3.customer_num) customer_num,sum(t3.total_payment) total_payment ,sum(t3.total_times) total_times,sum(t3.total_num)total_num
    from dw_rfm.b_tenant_rm_temp t3
    group by t3.tenant,t3.interval_type
) r;

-- 计算RM中客户占比，数据入目标表
insert overwrite table dw_rfm.b_active_customer_rm_temp partition(part='${stat_date}',level='tenant')
...
(
    select t.*,
	case when b.customer_num is null then 0 else b.customer_num end as bcusnum
    from dw_rfm.b_tenant_rm_result t
    left outer join
	(select tenant,interval_type,customer_num from dw_rfm.b_tenant_rm_result where recency =99 and monetary=99) b
    on t.tenant = b.tenant and t.interval_type=b.interval_type
) r;
