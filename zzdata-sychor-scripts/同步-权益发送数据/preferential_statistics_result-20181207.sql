...
CREATE TABLE IF NOT EXISTS dw_source.`s_preferential_statistics_temp`(
	`tenant` string,
    `uni_id` string,
    `total_num` int,
	`last_time` string
...
insert overwrite table dw_source.`s_preferential_statistics_temp`
select re.tenant,re.uni_id,sum(re.totalnum) as totalnum,max(re.lasttime) as lasttime 
from (
	...
	union all 
	select t1.tenant,t1.uni_id,-count(t1.uni_id) totalnum,max(t1.actiontime) as lasttime 
	from
	(
		 ...
		 on( a1.plat_code = b1.plat_code and a1.plat_account = b1.plat_account and a1.shop_id = b1.shop_id)
		 LEFT OUTER JOIN
...
-- 4、创建每日统计结果变化的表
CREATE TABLE IF NOT EXISTS dw_source.`s_preferential_statistics_result`(
	`tenant` string,
    `uni_id` string,
    `total_num` int,
	`last_time` string,
...
insert overwrite table dw_source.`s_preferential_statistics_result`
...
from (
	...
	from dw_source.s_preferential_statistics_temp a
	left outer join 
	dw_business.b_preferential_statistics_base b
