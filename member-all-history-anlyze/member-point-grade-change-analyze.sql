...
--创建积分变更临时表
CREATE TABLE IF NOT EXISTS dw_rfm.`b_point_change_analyze_monthend_temp`(
	`card_plan_id` string,
	`member_id` string,
	`change_value` bigint,
	`udtype` int,
...
insert overwrite table dw_rfm.b_point_change_analyze_monthend_temp partition(part)
...
-- 计算每个时间段内有积分变动的会员数
CREATE TABLE IF NOT EXISTS dw_rfm.`b_point_change_members_statics_monthend_temp`(
	`card_plan_id` string,
	`rangetype` int,
	`members` bigint
)
...
insert overwrite table dw_rfm.`b_point_change_members_statics_monthend_temp` partition(part)
select r.card_plan_id,r.rangetype,r.members,${hiveconf:thisMonthEnd} as part
from(
	select card_plan_id,1 as rangetype,count(distinct member_id) as members
	from dw_rfm.b_point_change_analyze_monthend_temp
	where part=${hiveconf:thisMonthEnd} and yestoday=1 group by card_plan_id
	union all
	select card_plan_id,2 as rangetype,count(distinct member_id) as members
	from dw_rfm.b_point_change_analyze_monthend_temp
	where part=${hiveconf:thisMonthEnd} and thisweek=1 group by card_plan_id
	union all
	select card_plan_id,3 as rangetype,count(distinct member_id) as members
	from dw_rfm.b_point_change_analyze_monthend_temp
	where part=${hiveconf:thisMonthEnd} and thismonth=1
	group by card_plan_id
	union all
	select card_plan_id,4 as rangetype,count(distinct member_id) as members
	from dw_rfm.b_point_change_analyze_monthend_temp
	where part=${hiveconf:thisMonthEnd} and last7day=1
	group by card_plan_id
	union all
	select card_plan_id,5 as rangetype,count(distinct member_id) as members
	from dw_rfm.b_point_change_analyze_monthend_temp
	where part=${hiveconf:thisMonthEnd} and last30day=1
	group by card_plan_id
)r;

-- 计算发放和消耗的积分 udtype=1 发放 udtype=-1 消耗
CREATE TABLE IF NOT EXISTS dw_rfm.`b_point_change_statics_monthend_temp`(
	`card_plan_id` string,
	`rangetype` int,
	`send_points` bigint,
...
insert overwrite table dw_rfm.`b_point_change_statics_monthend_temp` partition(part)
...
from (
	...
	(
		select card_plan_id,udtype,1 as rangetype,
			...
			from dw_rfm.b_point_change_analyze_monthend_temp
			where part=${hiveconf:thisMonthEnd} and yestoday=1 group by card_plan_id,udtype
		union all
			select card_plan_id,udtype,2 as rangetype,
			case when udtype=1 then sum(change_value) else abs(sum(change_value)) end as totalpoint
			from dw_rfm.b_point_change_analyze_monthend_temp
			where part=${hiveconf:thisMonthEnd} and thisweek=1 group by card_plan_id,udtype
		union all
			select card_plan_id,udtype,3 as rangetype,
			case when udtype=1 then sum(change_value) else abs(sum(change_value)) end as totalpoint
			from dw_rfm.b_point_change_analyze_monthend_temp
			where part=${hiveconf:thisMonthEnd} and thismonth=1 group by card_plan_id,udtype
		union all
			select card_plan_id,udtype,4 as rangetype,
			case when udtype=1 then sum(change_value) else abs(sum(change_value)) end as totalpoint
			from dw_rfm.b_point_change_analyze_monthend_temp
			where part=${hiveconf:thisMonthEnd} and last7day=1 group by card_plan_id,udtype
		union all
			select card_plan_id,udtype,5 as rangetype,
			case when udtype=1 then sum(change_value) else abs(sum(change_value)) end as totalpoint
			from dw_rfm.b_point_change_analyze_monthend_temp
			where part=${hiveconf:thisMonthEnd} and last30day=1 group by card_plan_id,udtype
	) t
) re
...
-- 近30天失效的积分计算
CREATE TABLE IF NOT EXISTS dw_rfm.`b_invalid_points_last30day_monthend`(
	`card_plan_id` string,
	`point` bigint,
	`yestoday` int,
	`thisweek` int,
...
insert overwrite table dw_rfm.`b_invalid_points_last30day_monthend` partition(part)
...
-- 几个时间段的过期积分总数
CREATE TABLE IF NOT EXISTS dw_rfm.`b_invalid_points_statics_monthend_temp`(
	`card_plan_id` string,
	`rangetype` int,
...
insert overwrite table dw_rfm.`b_invalid_points_statics_monthend_temp` partition(part)
select r.card_plan_id,r.rangetype,r.overdue_points,${hiveconf:thisMonthEnd} as part
from (
	select card_plan_id,1 as rangetype,sum(point) as overdue_points
		from dw_rfm.b_invalid_points_last30day_monthend
		where part=${hiveconf:thisMonthEnd} and yestoday=1 group by card_plan_id
	union all
	...
	union all
		...
		from dw_rfm.b_invalid_points_last30day_monthend
		where part=${hiveconf:thisMonthEnd} and thismonth=1 group by card_plan_id
	union all
		select card_plan_id,4 as rangetype,sum(point) as overdue_points
		from dw_rfm.b_invalid_points_last30day_monthend
		where part=${hiveconf:thisMonthEnd} and last7day=1 group by card_plan_id
	union all
		select card_plan_id,5 as rangetype,sum(point) as overdue_points
		from dw_rfm.b_invalid_points_last30day_monthend
		where part=${hiveconf:thisMonthEnd} and last30day=1 group by card_plan_id
)r;

-- 当前有效积分存量,不随时间变化,只有最新当前一份数据
CREATE TABLE IF NOT EXISTS dw_rfm.`b_current_effect_points_monthend_temp`(
	`card_plan_id` string,
...
insert overwrite table dw_rfm.`b_current_effect_points_monthend_temp` partition(part)
select card_plan_id,sum(point) effect_points,${hiveconf:thisMonthEnd} as part
	from dw_business.b_member_efffect_point
...
-- 从会员基本信息中查询新增的会员
CREATE TABLE IF NOT EXISTS dw_rfm.`b_new_member_last30day_monthend_temp`(
	`card_plan_id` string,
	`member_id` string,
	`yestoday` int,
	`thisweek` int,
...
insert overwrite table dw_rfm.`b_new_member_last30day_monthend_temp` partition(part)
...
-- 各时间段新增会员的统计结果
CREATE TABLE IF NOT EXISTS dw_rfm.`b_new_member_statics_monthend_temp`(
	`card_plan_id` string,
	`rangetype` int,
	`newmembers` bigint
)
...
insert overwrite table dw_rfm.`b_new_member_statics_monthend_temp` partition(part)
select r.card_plan_id,r.rangetype,r.newmembers,${hiveconf:thisMonthEnd} as part
from (
	select card_plan_id,1 as rangetype,count(member_id) as newmembers
		from dw_rfm.b_new_member_last30day_monthend_temp
		where part=${hiveconf:thisMonthEnd} and yestoday=1 group by card_plan_id
	union all
...
-- 计算会员等级变更数量
CREATE TABLE IF NOT EXISTS dw_rfm.`b_member_grade_statics_monthend_temp`(
	`card_plan_id` string,
	`rangetype` int,
	`upnums` bigint,
	`downnums` bigint
...
insert overwrite table dw_rfm.`b_member_grade_statics_monthend_temp` partition(part)
select re.card_plan_id,re.rangetype,sum(re.upnums) as upnums,sum(re.downnums) as downnums,${hiveconf:thisMonthEnd} as part
from(
...
insert overwrite table dw_rfm.`cix_online_member_point_grade_change` partition(part)
...
(
	...
	from (
		...
		left join 
		(select * from dw_rfm.b_current_effect_points_monthend_temp where part=${hiveconf:thisMonthEnd}) b
		on a.card_plan_id=b.card_plan_id
	) r
	lateral view explode(r.timetype) adtable as rangetype
) t
left join (select * from dw_rfm.b_point_change_members_statics_monthend_temp where part=${hiveconf:thisMonthEnd}) d
on t.card_plan_id = d.card_plan_id and t.rangetype = d.rangetype
left join (select * from dw_rfm.b_point_change_statics_monthend_temp where part=${hiveconf:thisMonthEnd}) e
on t.card_plan_id = e.card_plan_id and t.rangetype=e.rangetype
left join (select * from dw_rfm.b_invalid_points_statics_monthend_temp where part=${hiveconf:thisMonthEnd}) f
on t.card_plan_id = f.card_plan_id and t.rangetype=f.rangetype
left join (select * from dw_rfm.b_new_member_statics_monthend_temp where part=${hiveconf:thisMonthEnd}) h
on t.card_plan_id = h.card_plan_id and t.rangetype=h.rangetype
left join (select * from dw_rfm.b_member_grade_statics_monthend_temp where part=${hiveconf:thisMonthEnd}) g
...
CREATE TABLE IF NOT EXISTS dw_rfm.`b_yestoday_member_grade_change_monthend_temp`(
	`card_plan_id` string,
	`nowgrade` int,
	`oldgrade` int,
	`members` bigint
...
insert overwrite table dw_rfm.`b_yestoday_member_grade_change_monthend_temp` partition(part)
select re.card_plan_id,re.nowgrade,re.oldgrade,count(re.member_id) members,${hiveconf:thisMonthEnd} as part
from (
...
from (
	...
	from dw_rfm.b_yestoday_member_grade_change_monthend_temp r where r.part=${hiveconf:thisMonthEnd}
	union all
	select r1.card_plan_id,r1.nowgrade,99 as oldgrade,sum(r1.members) members 
	from dw_rfm.b_yestoday_member_grade_change_monthend_temp r1 where r1.part=${hiveconf:thisMonthEnd}
...
-- 本周等级变化
CREATE TABLE IF NOT EXISTS dw_rfm.`b_thisweek_member_grade_change_monthend_temp`(
	`card_plan_id` string,
	`nowgrade` int,
	`oldgrade` int,
	`members` bigint
...
insert overwrite table dw_rfm.b_thisweek_member_grade_change_monthend_temp partition(part)
select re.card_plan_id,re.nowgrade,re.oldgrade,count(re.member_id) members,${hiveconf:thisMonthEnd} as part
from (
	select t.card_plan_id,t.member_id,t.grade as nowgrade,
		case when t.member_type=-1 then -1 --非会员
...
from(
	select r.card_plan_id,r.nowgrade,r.oldgrade,r.members 
	from dw_rfm.b_thisweek_member_grade_change_monthend_temp r where r.part=${hiveconf:thisMonthEnd}
	union all
	select r1.card_plan_id,r1.nowgrade,99 as oldgrade,sum(r1.members) members 
	from dw_rfm.b_thisweek_member_grade_change_monthend_temp r1 where r1.part=${hiveconf:thisMonthEnd}
	group by r1.card_plan_id,r1.nowgrade
)re;
...
-- 本月等级变化
CREATE TABLE IF NOT EXISTS dw_rfm.`b_thismonth_member_grade_change_monthend_temp`(
	`card_plan_id` string,
	`nowgrade` int,
	`oldgrade` int,
	`members` bigint
...
insert overwrite table dw_rfm.b_thismonth_member_grade_change_monthend_temp partition(part)
select re.card_plan_id,re.nowgrade,re.oldgrade,count(re.member_id) members,${hiveconf:thisMonthEnd} as part
from (
	select t.card_plan_id,t.member_id,t.grade as nowgrade,
		case when t.member_type=-1 then -1 --非会员
...
from(
	select r.card_plan_id,r.nowgrade,r.oldgrade,r.members 
	from dw_rfm.b_thismonth_member_grade_change_monthend_temp r where r.part=${hiveconf:thisMonthEnd}
	union all
	select r1.card_plan_id,r1.nowgrade,99 as oldgrade,sum(r1.members) members 
	from dw_rfm.b_thismonth_member_grade_change_monthend_temp r1 where r1.part=${hiveconf:thisMonthEnd}
	group by r1.card_plan_id,r1.nowgrade
)re;
...
-- 近7天等级变化
CREATE TABLE IF NOT EXISTS dw_rfm.`b_last7day_member_grade_change_monthend_temp`(
	`card_plan_id` string,
	`nowgrade` int,
	`oldgrade` int,
	`members` bigint
...
-- 增加列合计计算
insert overwrite table dw_rfm.b_last7day_member_grade_change_monthend_temp partition(part)
select re.card_plan_id,re.nowgrade,re.oldgrade,re.members,${hiveconf:thisMonthEnd} as part
from(
	select r.card_plan_id,r.nowgrade,r.oldgrade,r.members 
	from dw_rfm.b_last7day_member_grade_change_monthend_temp r where r.part=${hiveconf:thisMonthEnd}
	union all
	select r1.card_plan_id,r1.nowgrade,99 as oldgrade,sum(r1.members) members 
	from dw_rfm.b_last7day_member_grade_change_monthend_temp r1 where r1.part=${hiveconf:thisMonthEnd}
	group by r1.card_plan_id,r1.nowgrade
)re;
...
-- 近30天等级变化
CREATE TABLE IF NOT EXISTS dw_rfm.`b_last30day_member_grade_change_monthend_temp`(
	`card_plan_id` string,
	`nowgrade` int,
	`oldgrade` int,
	`members` bigint
...
insert overwrite table dw_rfm.b_last30day_member_grade_change_monthend_temp partition(part)
select re.card_plan_id,re.nowgrade,re.oldgrade,count(re.member_id) members,${hiveconf:thisMonthEnd} as part
from (
	select t.card_plan_id,t.member_id,t.grade as nowgrade,
		case when t.member_type=-1 then -1 --非会员
...
from(
	select r.card_plan_id,r.nowgrade,r.oldgrade,r.members 
	from dw_rfm.b_last30day_member_grade_change_monthend_temp r where r.part=${hiveconf:thisMonthEnd}
	union all
	select r1.card_plan_id,r1.nowgrade,99 as oldgrade,sum(r1.members) members 
	from dw_rfm.b_last30day_member_grade_change_monthend_temp r1 where r1.part=${hiveconf:thisMonthEnd}
	group by r1.card_plan_id,r1.nowgrade
)re;
...
insert overwrite table dw_rfm.cix_online_member_grade_transform partition(part)
select r.card_plan_id,r.nowgrade,r.oldgrade,r.members,1 as rangetype,
	${hiveconf:thisMonthEnd} as stat_date,${hiveconf:submitTime} as modified,${hiveconf:thisMonthEnd} as part
from dw_rfm.b_yestoday_member_grade_change_monthend_temp r where r.part=${hiveconf:thisMonthEnd};

insert into table dw_rfm.cix_online_member_grade_transform partition(part)
select r1.card_plan_id,r1.nowgrade,r1.oldgrade,r1.members,2 as rangetype,
${hiveconf:thisMonthEnd} as stat_date,${hiveconf:submitTime} as modified,${hiveconf:thisMonthEnd} as part
from dw_rfm.b_thisweek_member_grade_change_monthend_temp r1 where r1.part=${hiveconf:thisMonthEnd};

insert into table dw_rfm.cix_online_member_grade_transform partition(part)
select r2.card_plan_id,r2.nowgrade,r2.oldgrade,r2.members,3 as rangetype,
	${hiveconf:thisMonthEnd} as stat_date,${hiveconf:submitTime} as modified,${hiveconf:thisMonthEnd} as part
from dw_rfm.b_thismonth_member_grade_change_monthend_temp r2 where r2.part=${hiveconf:thisMonthEnd};

insert into table dw_rfm.cix_online_member_grade_transform partition(part)
select r3.card_plan_id,r3.nowgrade,r3.oldgrade,r3.members,4 as rangetype,
	${hiveconf:thisMonthEnd} as stat_date,${hiveconf:submitTime} as modified,${hiveconf:thisMonthEnd} as part
from dw_rfm.b_last7day_member_grade_change_monthend_temp r3 where r3.part=${hiveconf:thisMonthEnd};

insert into table dw_rfm.cix_online_member_grade_transform partition(part)
select r4.card_plan_id,r4.nowgrade,r4.oldgrade,r4.members,5 as rangetype,
	${hiveconf:thisMonthEnd} as stat_date,${hiveconf:submitTime} as modified,${hiveconf:thisMonthEnd} as part
from dw_rfm.b_last30day_member_grade_change_monthend_temp r4 where r4.part=${hiveconf:thisMonthEnd};

-- 删除临时近30天的会员临时表、过期积分的统计
--alter table dw_rfm.b_invalid_points_last30day_monthend drop partition (part=${hiveconf:thisMonthEnd});
--alter table dw_rfm.b_new_member_last30day_monthend_temp drop partition (part=${hiveconf:thisMonthEnd});
-- 删除中间临时表
--alter table dw_rfm.b_point_change_analyze_monthend_temp drop partition (part=${hiveconf:thisMonthEnd});
--alter table dw_rfm.b_yestoday_member_grade_change_monthend_temp drop partition (part=${hiveconf:thisMonthEnd});
--alter table dw_rfm.b_thisweek_member_grade_change_monthend_temp drop partition (part=${hiveconf:thisMonthEnd});
--alter table dw_rfm.b_thismonth_member_grade_change_monthend_temp drop partition (part=${hiveconf:thisMonthEnd});
--alter table dw_rfm.b_last7day_member_grade_change_monthend_temp drop partition (part=${hiveconf:thisMonthEnd});
--alter table dw_rfm.b_last30day_member_grade_change_monthend_temp drop partition (part=${hiveconf:thisMonthEnd});
