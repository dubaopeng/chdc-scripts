SET mapred.job.name='member-life-cycle-monthend-analyze-历史月底会员生命周期分析';
...
-- 店铺级的看客户
CREATE TABLE IF NOT EXISTS dw_rfm.`b_shop_card_customer_monthend_temp`(
	`tenant` string,
	`plat_code` string,
	`uni_shop_id` string,
	`uni_id` string,
...
insert overwrite table dw_rfm.b_shop_card_customer_monthend_temp partition(part)
...
-- 店铺级的看客户
CREATE TABLE IF NOT EXISTS dw_rfm.`b_shop_all_customer_life_monthend_count`(
	`tenant` string,
	`plat_code` string,
	`uni_shop_id` string,
	`prospective` bigint,
...
--将每个店铺所有客户的生命周期统计出来
insert overwrite table dw_rfm.b_shop_all_customer_life_monthend_count partition(part)
...
from (
	...
	from(
		...
		from 
			dw_rfm.b_shop_card_customer_monthend_temp t where t.part=${hiveconf:thisMonthEnd} 
...
insert overwrite table dw_rfm.cix_online_member_life_cycle partition(part)
...
from (
	...
	from(
		...
		from 
			dw_rfm.b_shop_card_customer_monthend_temp t
		where t.part=${hiveconf:thisMonthEnd} and t.grade is not null
...
-- 计算会员卡下会员的合计行
CREATE TABLE IF NOT EXISTS dw_rfm.`b_shop_member_count_monthend_temp`(
	`tenant` string,
	`plat_code` string,
	`uni_shop_id` string,
	`card_plan_id` string,
...
insert overwrite table dw_rfm.b_shop_member_count_monthend_temp partition(part)
...
where t.part=${hiveconf:thisMonthEnd} and t.card_plan_id is not null
group by t.tenant,t.plat_code,t.uni_shop_id,t.card_plan_id;

-- 计算店铺中各会员卡的非会员数量，店铺客户总指标-店铺会员的指标
CREATE TABLE IF NOT EXISTS dw_rfm.`b_shop_member_total_monthend_temp`(
	`tenant` string,
	`plat_code` string,
	`uni_shop_id` string,
	`card_plan_id` string,
...
insert overwrite table dw_rfm.b_shop_member_total_monthend_temp partition(part)
...
(select tenant,plat_code,uni_shop_id,prospective,active_new,phurce_new,active_old,phurce_old,silent,loss,whole
 from dw_rfm.b_shop_all_customer_life_monthend_count where part=${hiveconf:thisMonthEnd}
) a
left join 
(select tenant,plat_code,uni_shop_id,card_plan_id,prospective,active_new,phurce_new,active_old,phurce_old,silent,loss,whole
  from dw_rfm.b_shop_member_count_monthend_temp where part=${hiveconf:thisMonthEnd}
)b
on a.tenant=b.tenant and a.plat_code=b.plat_code and a.uni_shop_id=b.uni_shop_id;

--店铺级的会员行和店铺级的非会员行
insert into table dw_rfm.cix_online_member_life_cycle partition(part)
select a.tenant,a.plat_code,a.uni_shop_id,a.card_plan_id,99 as grade,
	   ...
	   ${hiveconf:submitTime} as modified,
	   a.part
	   from dw_rfm.b_shop_member_total_monthend_temp a
	   where a.part=${hiveconf:thisMonthEnd}
union all
select b.tenant,b.plat_code,b.uni_shop_id,b.card_plan_id,-1 as grade,
	   ...
	   ${hiveconf:submitTime} as modified,
	   b.part
	   from dw_rfm.b_shop_member_total_monthend_temp b
	   where b.part=${hiveconf:thisMonthEnd}
union all
select c.tenant,c.plat_code,c.uni_shop_id,c.card_plan_id,c.grade,
	   ...
	   ${hiveconf:submitTime} as modified,
	   c.part
	   from dw_rfm.b_shop_member_count_monthend_temp c
	   where c.part=${hiveconf:thisMonthEnd};


-- 基于店铺级卡客户关系数据，计算平台级数据和租户级数据
CREATE TABLE IF NOT EXISTS dw_rfm.`b_plat_card_customer_monthend_temp`(
	`tenant` string,
	`plat_code` string,
	`uni_id` string,
	`card_plan_id` string,
...
insert overwrite table dw_rfm.b_plat_card_customer_monthend_temp partition(part)
...
from(
	...
	from dw_rfm.b_qqd_plat_rfm where part=${hiveconf:thisMonthEnd}
) t
left join 
(
	select tenant,plat_code,uni_id,card_plan_id,member_id,grade
	from dw_rfm.b_shop_card_customer_monthend_temp where part=${hiveconf:thisMonthEnd}
)m
on t.tenant=m.tenant and t.plat_code = m.plat_code and t.uni_id = m.uni_id;


--将每个平台所有客户的生命周期统计出来
CREATE TABLE IF NOT EXISTS dw_rfm.`b_plat_all_customer_life_monthend_count`(
	`tenant` string,
	`plat_code` string,
	`prospective` bigint,
	`active_new` bigint,
...
--将每个平台的所有客户的生命周期统计出来
insert overwrite table dw_rfm.b_plat_all_customer_life_monthend_count partition(part)
...
from (
	...
	from(
		...
		from 
			dw_rfm.b_plat_card_customer_monthend_temp t where t.part=${hiveconf:thisMonthEnd}
...
-- 创建平台级的会员生命周期
CREATE TABLE IF NOT EXISTS dw_rfm.`b_plat_member_life_cycle_monthend_temp`(
	`tenant` string,
	`plat_code` string,
	`card_plan_id` string,
	`grade` int, 
...
insert overwrite table dw_rfm.b_plat_member_life_cycle_monthend_temp partition(part)
...
from (
	...
	from(
		...
		from 
			dw_rfm.b_plat_card_customer_monthend_temp t
		where t.part=${hiveconf:thisMonthEnd} and t.grade is not null
...
-- 计算平台+会员卡下会员的合计行
CREATE TABLE IF NOT EXISTS dw_rfm.`b_plat_member_count_monthend_temp`(
	`tenant` string,
	`plat_code` string,
	`card_plan_id` string,
	`grade` int,
...
insert overwrite table dw_rfm.b_plat_member_count_monthend_temp partition(part)
select t.tenant,t.plat_code,t.card_plan_id,98 as grade,
	  ...
	  sum(t.whole) as whole,
	  ${hiveconf:thisMonthEnd} as part
from dw_rfm.b_plat_member_life_cycle_monthend_temp t
where t.part=${hiveconf:thisMonthEnd} and t.card_plan_id is not null
group by t.tenant,t.plat_code,t.card_plan_id;

-- 计算平台中各会员卡的非会员数量，平台客户总指标-平台会员的指标
CREATE TABLE IF NOT EXISTS dw_rfm.`b_plat_member_total_monthend_temp`(
	`tenant` string,
	`plat_code` string,
	`card_plan_id` string,
	`prospective` bigint,
...
insert overwrite table dw_rfm.b_plat_member_total_monthend_temp partition(part)
...
(select tenant,plat_code,prospective,active_new,phurce_new,active_old,phurce_old,silent,loss,whole
	from dw_rfm.b_plat_all_customer_life_monthend_count where part=${hiveconf:thisMonthEnd}) a
left join 
(select tenant,plat_code,card_plan_id,prospective,active_new,phurce_new,active_old,phurce_old,silent,loss,whole
	from dw_rfm.b_plat_member_count_monthend_temp where part=${hiveconf:thisMonthEnd}) b
on a.tenant=b.tenant and a.plat_code=b.plat_code;

--平台级的会员行和平台级的非会员行
insert into table dw_rfm.b_plat_member_life_cycle_monthend_temp partition(part)
select a.tenant,a.plat_code,a.card_plan_id,99 as grade,
   a.prospective,a.active_new,a.phurce_new,a.active_old,a.phurce_old,a.silent,a.loss,a.whole,a.part
   from dw_rfm.b_plat_member_total_monthend_temp a where a.part=${hiveconf:thisMonthEnd}
union all
select b.tenant,b.plat_code,b.card_plan_id,-1 as grade,
	   b.ng_prospective,b.ng_active_new,b.ng_phurce_new,b.ng_active_old,b.ng_phurce_old,b.ng_silent,b.ng_loss,b.ng_whole,b.part
	   from dw_rfm.b_plat_member_total_monthend_temp b where b.part=${hiveconf:thisMonthEnd}
union all
select c.tenant,c.plat_code,c.card_plan_id,c.grade,
	   c.prospective,c.active_new,c.phurce_new,c.active_old,c.phurce_old,c.silent,c.loss,c.whole,c.part
	   from dw_rfm.b_plat_member_count_monthend_temp c where c.part=${hiveconf:thisMonthEnd};
	   
	   
-- 基于平台级卡客户关系数据，计算租户级数据和租户级数据
CREATE TABLE IF NOT EXISTS dw_rfm.`b_tenant_card_customer_monthend_temp`(
	`tenant` string,
	`uni_id` string,
	`card_plan_id` string,
	`member_id` string,
...
insert overwrite table dw_rfm.b_tenant_card_customer_monthend_temp partition(part)
...
from(
	select tenant,uni_id,earliest_time,first_buy_time,year_buy_times,
			tyear_buy_times,btyear_buy_times
	from dw_rfm.b_qqd_tenant_rfm where part=${hiveconf:thisMonthEnd}
) t
left join 
(select tenant,uni_id,card_plan_id,member_id,grade from dw_rfm.b_plat_card_customer_monthend_temp where part=${hiveconf:thisMonthEnd}) m
on t.tenant=m.tenant and t.uni_id = m.uni_id;


--将每个租户所有客户的生命周期统计出来
CREATE TABLE IF NOT EXISTS dw_rfm.`b_tenant_all_customer_life_monthend_count`(
	`tenant` string,
	`prospective` bigint,
	`active_new` bigint,
	`phurce_new` bigint,
...
insert overwrite table dw_rfm.b_tenant_all_customer_life_monthend_count partition(part)
select c.tenant,
	if(c.prospective is null,0,c.prospective) as prospective,
	if(c.active_new is null,0,c.active_new) active_new,
	if(c.phurce_new is null,0,c.phurce_new) phurce_new,
...
from (
	...
	from(
		...
		from 
			dw_rfm.b_tenant_card_customer_monthend_temp t
...
-- 创建租户级的会员生命周期
CREATE TABLE IF NOT EXISTS dw_rfm.`b_tenant_member_life_cycle_monthend_temp`(
	`tenant` string,
	`card_plan_id` string,
	`grade` int, 
    `prospective` bigint, 
...
-- 计算每个平台会员卡中各等级对应的客户类型的数量
insert overwrite table dw_rfm.b_tenant_member_life_cycle_monthend_temp partition(part)
...
-- 计算租户+会员卡下会员的合计行
CREATE TABLE IF NOT EXISTS dw_rfm.`b_tenant_member_count_monthend_temp`(
	`tenant` string,
	`card_plan_id` string,
	`grade` int,
	`prospective` bigint,
...
insert overwrite table dw_rfm.b_tenant_member_count_monthend_temp partition(part)
select t.tenant,t.card_plan_id,98 as grade,
	  ...
	  sum(t.whole) as whole,
	  ${hiveconf:thisMonthEnd} as part
from dw_rfm.b_tenant_member_life_cycle_monthend_temp t
where t.part=${hiveconf:thisMonthEnd} and t.card_plan_id is not null
group by t.tenant,t.card_plan_id;

-- 计算租户中各会员卡的非会员数量，租户客户总指标-平台会员的指标
CREATE TABLE IF NOT EXISTS dw_rfm.`b_tenant_member_total_monthend_temp`(
	`tenant` string,
	`card_plan_id` string,
	`prospective` bigint,
	`active_new` bigint,
...
insert overwrite table dw_rfm.b_tenant_member_total_monthend_temp partition(part)
...
(select tenant,prospective,active_new,phurce_new,active_old,phurce_old,silent,loss,whole
	from dw_rfm.b_tenant_all_customer_life_monthend_count where part=${hiveconf:thisMonthEnd}) a
left join 
(select tenant,card_plan_id,prospective,active_new,phurce_new,active_old,phurce_old,silent,loss,whole
	from dw_rfm.b_tenant_member_count_monthend_temp where part=${hiveconf:thisMonthEnd}) b
on a.tenant=b.tenant;

--租户级的会员行和平台级的非会员行
insert into table dw_rfm.b_tenant_member_life_cycle_monthend_temp partition(part)
select a.tenant,a.card_plan_id,99 as grade,
	   a.prospective,a.active_new,a.phurce_new,a.active_old,a.phurce_old,a.silent,a.loss,a.whole,a.part
	   from dw_rfm.b_tenant_member_total_monthend_temp a where a.part=${hiveconf:thisMonthEnd}
union all
select b.tenant,b.card_plan_id,-1 as grade,
	   b.ng_prospective,b.ng_active_new,b.ng_phurce_new,b.ng_active_old,b.ng_phurce_old,b.ng_silent,b.ng_loss,b.ng_whole,b.part
	   from dw_rfm.b_tenant_member_total_monthend_temp b where b.part=${hiveconf:thisMonthEnd}
union all
select c.tenant,c.card_plan_id,c.grade,
	   c.prospective,c.active_new,c.phurce_new,c.active_old,c.phurce_old,c.silent,c.loss,c.whole,c.part
	   from dw_rfm.b_tenant_member_count_monthend_temp c where c.part=${hiveconf:thisMonthEnd};
...
select t1.tenant,null as plat_code,null as uni_shop_id,t1.card_plan_id,t1.grade,
	  ...
	  ${hiveconf:submitTime} as modified,
	  ${hiveconf:thisMonthEnd} as part
from dw_rfm.b_tenant_member_life_cycle_monthend_temp t1 
where t1.part=${hiveconf:thisMonthEnd} and t1.card_plan_id is not null;


-- 删除中间临时表
--drop table if exists dw_rfm.b_shop_card_customer_monthend_temp;
--drop table if exists dw_rfm.b_shop_member_total_monthend_temp;
--drop table if exists dw_rfm.b_shop_member_count_monthend_temp;
--drop table if exists dw_rfm.b_shop_all_customer_life_monthend_count;

--drop table if exists dw_rfm.b_plat_card_customer_monthend_temp;
--drop table if exists dw_rfm.b_plat_member_total_monthend_temp;
--drop table if exists dw_rfm.b_plat_member_count_monthend_temp;
--drop table if exists dw_rfm.b_plat_all_customer_life_monthend_count;
--drop table if exists dw_rfm.b_plat_member_life_cycle_monthend_temp;

--drop table if exists dw_rfm.b_tenant_card_customer_monthend_temp;
--drop table if exists dw_rfm.b_tenant_member_total_monthend_temp;
--drop table if exists dw_rfm.b_tenant_member_count_monthend_temp;
--drop table if exists dw_rfm.b_tenant_all_customer_life_monthend_count;
--drop table if exists dw_rfm.b_tenant_member_life_cycle_monthend_temp;
