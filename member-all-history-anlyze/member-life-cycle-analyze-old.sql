...
CREATE TABLE IF NOT EXISTS dw_rfm.`b_customer_member_monthend_rfm_temp`(
	`tenant` string,
	`plat_code` string,
	`uni_shop_id` string,
	`uni_id` string,
...
insert overwrite table dw_rfm.`b_customer_member_monthend_rfm_temp` partition(part)
...
from (
	...
	from(
		...
		from 
			dw_rfm.b_customer_member_monthend_rfm_temp t
		where t.part=${hiveconf:thisMonthEnd} and t.ismember=0
	...
	from(
		...
		from 
			dw_rfm.b_customer_member_monthend_rfm_temp t
		where t.part=${hiveconf:thisMonthEnd} and t.ismember=-1
...
where t.part=${hiveconf:thisMonthEnd} and t.grade > -1 
...
from dw_rfm.cix_online_member_life_cycle t1
	where t1.part=${hiveconf:thisMonthEnd} --合计行数据
...
-- 删除中间临时表
--alter table dw_rfm.b_customer_member_monthend_rfm_temp drop partition (part=${hiveconf:thisMonthEnd});
