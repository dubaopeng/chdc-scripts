...
create table IF NOT EXISTS dw_dw_event.festive_care_shop (
	shopId string,
	config string
)
...
select shopId from dw_event.festive_care_shop
...
left outer join
(
	-- select buyer_nick,dp_id,status,grade from dw_base.b_top_member
	select uni_shop_id as dp_id,name as buyer_nick,'' as status,grade 
	from dw_business.b_std_member_base_info 
	where plat_code='TAOBAO'
) as m
on concat('TAOBAO','|',s.shopId)=m.dp_id
where m.buyer_nick is not null;
...
from dw_event.festive_care_effective_member as member
left outer join (
	-- select buyer_nick,full_name,mobile from dw_base.b_top_customer
	select name as buyer_nick,
	
) as customer 
on member.buyer_nick=customer.buyer_nick
